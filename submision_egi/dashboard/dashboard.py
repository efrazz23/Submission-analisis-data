...
# --- 4. LOGIKA FILTER (SIDEBAR TETAP ADA, TAPI GAK MAKSIMALIN HARUS INPUT) ---
...
with st.sidebar:
    ...
    # Menggunakan try-except agar jika user baru klik 1 tanggal, dashboard tidak error/hilang
    try:
...
# Eksekusi filter (Jika start/end sesuai, main_df akan terpotong, jika tidak dia tetap all_df)
...
with col1:
    ...
    ax.set_title("Top 5 States", fontsize=20)
    st.pyplot(fig)
with col2:
    fig, ax = plt.subplots(figsize=(15, 8))
    sns.barplot(x="customer_count", y="customer_city", data=bycity_df.head(5), palette="Blues_d", ax=ax)
    ax.set_title("Top 5 Cities", fontsize=20)
    st.pyplot(fig)

# 3. Delivery Time Analysis
st.subheader("Delivery Time Analysis")
col1, col2 = st.columns(2)
col1.metric("Avg Delivery Time (Days)", value=f"{round(delivery_time_df['delivery_time_days'].mean(), 1)} Days")
col2.metric("Max Delivery Delay (Days)", value=f"{round(delivery_time_df['delivery_time_days'].max(), 1)} Days")

# 4. Best & Worst Performing Product
...
sns.barplot(x="price", y="product_category_name", data=sum_order_items_df.sort_values(by="order_id", ascending=True).head(5), palette=colors, ax=ax[1])
ax[1].set_title("Worst Performing (Volume)", fontsize=50)
ax[1].invert_xaxis()
...
col3.metric("Avg Monetary", value=format_currency(rfm_df.monetary.mean(), "USD", locale='es_CO'))

st.caption('Copyright (c) Dicoding Submission 2026 | Egi Farhan')
